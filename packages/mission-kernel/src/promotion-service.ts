...
export class PromotionService {
  ...
  async promote(input: PromotionInput): Promise<PromotionResult> {
    ...
    return this.options.workspaceService.promote(input.workspace, input.mission.targetBranch, input.reviewerId.trim(), input.changeSnapshot);
  }
  ...
  async preparePromotion(input: PromotionInput): Promise<PromotionPreparation> {
    ...
    return this.options.workspaceService.preparePromotion(
      input.workspace,
  ...
  commitPromotion(token: PromotionRetryToken, reviewerId: string): Promise<PromotionResult> {
    return this.promoteRetry(token, reviewerId);
  }
  ...
  reconcilePromotion(token: PromotionRetryToken): Promise<PromotionReconciliation> {
    ...
    return this.options.workspaceService.reconcilePromotion(token);
  }
  ...
  async promoteRetry(token: PromotionRetryToken, reviewerId: string): Promise<PromotionResult> {
    ...
    return this.options.workspaceService.promoteRetry(token, reviewerId.trim());
  }
}
