...
describe("mission control frontend container module", () => {
  it("registers one widget factory per view without unbound constructor dependencies", () => {
    ...
    expect(factories().map(factory => factory.id).sort()).toEqual([ORRERY_INTELLIGENCE_WIDGET_ID, MISSION_CONTROL_WIDGET_ID].sort());
  });

  it.each([MISSION_CONTROL_WIDGET_ID, ORRERY_INTELLIGENCE_WIDGET_ID])("creates a fresh %s widget after the previous widget is disposed", async (id) => {
